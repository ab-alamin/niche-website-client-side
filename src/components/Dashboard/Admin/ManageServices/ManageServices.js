...
const ManageServices = () => {
  ...
  const handleDeleteService = (id) => {
    fetch(`https://infinite-wildwood-81897.herokuapp.com/services/${id}`, {
      method: "DELETE",
  ...
  const handleEditInfo = (id) => {
    fetch(`https://infinite-wildwood-81897.herokuapp.com/serviceUpdate/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setEditService(data);
        setEditForm(true);
  ...
  const handleServiceEdit = (id) => {
    ...
    fetch(`https://infinite-wildwood-81897.herokuapp.com/update/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
